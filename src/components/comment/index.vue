    ...
    <div class="comment-body">
      ...
      <div class="content">
        ...
        <div class="cell" v-if="commentList.length" v-for="(item, index) in commentList" :key="index">
          <div class="left">
            <img :src="item.avatar || defaultHeadPic" alt="headPic" @error="headPicError">
          </div>
          <div class="right">
            <p class="comment-author">{{item.userName | userNameFilter}}</p>
            <p class="comment-detail">{{item.content}}</p>
            <!-- 这条评论对应的回复 -->
            <div class="comment-reply" v-if="item.child && item.child.length">
            ...
            <p class="comment-footer">
              <span class="time">{{item.updateTime | timeFilte}}</span>
              <span class="delete" v-if="item.userId == userId" @click="del(item.id, '')">删除</span>
              <span class="zan" @click="supportComment(item)">赞<svg-icon :iconClass="'zan'"></svg-icon> ({{item.ups}})</span>
              <span class="reply" @click="openReplyInput(item)">回复<svg-icon :iconClass="'reply'"></svg-icon></span>
            </p>
            <!-- 回复 -> 评论 -->
            <div class="reply-content" v-if="item.replyInputShow">
      ...
      <div class="loadmore" v-if="commentList.length">
...
export default {
  ...
  methods: {
    ...
    getComment() {
      return this.fetchCommentPage(this.pageParams).then(res => {
        console.log(res)
