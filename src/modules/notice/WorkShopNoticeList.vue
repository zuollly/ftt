<template>
    <div class="work-notice-wrapper">
      <div class="work-notice-mainBody">
        <div v-if="homeNoticeList.length === 0">
          <LayNotData gridType="tb"></LayNotData>
        </div>
        <div class="home-notice" v-if="page === 'home' && homeNoticeList.length > 0">
          <el-table :data="homeNoticeList">
        ...
        <div class="work-notice-list-wrapper" v-if="page !== 'home' && homeNoticeList.length > 0">
          <el-table :data="homeNoticeList" style="width: 100%">
...
export default {
  ...
  watch: {
    homeNoticeList: function(val) {
      if (this.page === 'home' && val.length > 5) {
        this.homeNoticeList.length = 5
      }
    }
  },
  methods: {
    opeNotice(item, type) {
      ...
      if (type === 'view') {
        this.$router.push({
          name: 'noticeShow', params: { id: item.id }, query: { contentId: item.contentId, categoryMap: '公告', type: 'content' }
        })
      }
    },
    // 删除公告
