  ...
  <div v-loading="loading">
    <el-dialog title="选择成员" append-to-body width="80%" :visible.sync="dialogTableVisible" :before-close="beforeClose">
      ...
      <div style="margin-top: 20px">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="teacherParams.pageNum" :page-sizes="[10, 20, 30, 50]" :page-size="teacherParams.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total">
        </el-pagination>
      </div>
...
<script>
import { getArea, getSchool, getTeacherList } from '@/api/workshop'
...
export default {
  ...
  methods: {
    ...
    sure() {
      ...
      this.$emit('memberList', this.multipleSelection)
    },
    cancel() {
      this.$emit('memberList', this.multipleSelection)
    },
    beforeClose() {
      this.$emit('memberList')
    },
